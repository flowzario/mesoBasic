                 ...
                 const GetPot& input_params) : p(pin), c(p)
{
    ...
    deli = (nz+2)*(ny+2);
	 delj = (nz+2);
	 delk = 1;
	 bx = input_params("PFApp/bx",1);
	 by = input_params("PFApp/by",1);
	 bz = input_params("PFApp/bz",1);
    mobStep = input_params("PFApp/mobStep",1);
...
void TIPSphil::updatePhaseField()
{
    ...
    for (int i=1; i<nx+1; i++) {
        for (int j=1; j<ny+1; j++) {
            for (int k=1; k<nz+1; k++) {
                ...
                D = D0 * (T/Tinit);
                if (D > D0) D = D0;
		          double Dp = D * exp (- gamma * pow(cc_phil,nu)); // phillies diffusion
                // ----------------------
                // mobility
                // ----------------------
                double Mc = Dp/ddf;
   ...
   // print mobility layout
    if (current_step == mobStep) {
         int iskip = p.iskip;
         int jskip = p.jskip;
         int kskip = p.kskip;
         mob.writeVTKFile("mob",current_step,iskip,jskip,kskip);
    }    
    // ---------------------------------------
    // update CH equation:
    // ---------------------------------------
